...
{
	// Variable de entorno inicia con $
	if (arg[0] == '$') {
		// Variable de entorno magica
		if (arg[1] == '?' && arg[2] == '\0') {
		...
		if (value != NULL) {  // Si existe la variable
			arg = realloc(arg,
			              strlen(value) +
			                      1);  // Añade mas bloques de memoria para la ruta
			strcpy(arg, value);  // Copia la ruta para el nuevo bloque
		} else {
			free(arg);  // Libera la variable que no existe guardada en el heap
			arg = strdup("");  // Lo cambia por ""
		}
	}
...
parse_line(char *buf)
{
	...
	char *right = split_line(buf, pipe);

	l = parse_cmd(buf);

	if (block_contains(right, pipe) >= 0) {
		r = parse_line(right);
	} else {
		r = parse_cmd(right);
	}
